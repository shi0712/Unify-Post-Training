...
"""
A lightweight one-file FSDP SFT Trainer with phi-function per-token weighting.
Based on fsdp_sft_trainer.py with added prob_coefficients weighting.
"""
...
class FSDPDFTTrainer(object):
    """FSDP SFT Trainer with phi-function per-token probability weighting."""
    ...
    def _compute_loss(self, batch):
        ...
        # Phi-function per-token weighting: weight = p(correct_token)
    ...
    def training_step(self, batch: TensorDict):
        ...
        for micro_batch in micro_batches:
            loss, original_loss = self._compute_loss(batch=micro_batch)
            loss = loss / n_micro_batches
            loss.backward()
            step_loss += loss.item()
            step_original_loss += original_loss.item() / n_micro_batches
        ...
        step_original_loss = torch.tensor(step_original_loss).cuda()
        torch.distributed.all_reduce(step_loss, op=torch.distributed.ReduceOp.AVG)
        torch.distributed.all_reduce(step_original_loss, op=torch.distributed.ReduceOp.AVG)
        return {
            'train/loss': step_loss.detach().item(),
            'train/original_loss': step_original_loss.detach().item(),
    ...
    def validation_step(self, batch: TensorDict):
        ...
        with torch.no_grad():
            loss, original_loss = self._compute_loss(batch)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in self.train_dataloader:
                ...
                if global_step >= self.total_training_steps:
                    ...
                    if rank == 0:
                        ...
                        avg_val_original_loss = torch.mean(torch.stack(val_original_losses))
                        metric = {
                            'val/loss': avg_val_loss.detach().item(),
                            'val/original_loss': avg_val_original_loss.detach().item(),
                        }
                        tracking.log(data=metric, step=global_step)
                    torch.distributed.barrier()
            ...
            for data in self.val_dataloader:
                ...
                val_loss, val_original_loss = self.validation_step(data)
                val_losses.append(val_loss)
                val_original_losses.append(val_original_loss)
            if rank == 0:
                ...
                metric = {
                    ...
                    'val/original_loss': val_original_loss.detach().item(),
                }
                tracking.log(data=metric, step=global_step)
            torch.distributed.barrier()
...
def main(config):
    ...
    trainer = FSDPDFTTrainer(config=config, device_mesh=device_mesh)
    trainer.fit()
